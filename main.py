...
if submit_btn:
    ...
    sl.write("###")
    ner_entities = extract_entities(article.text)
    filtered_entities = [(entity, label) for entity, label in ner_entities if label in ["NORP", "PERSON", "ORG", "LOC"]]
    custom_order = ["ORG", "PERSON", "LOC", "NORP"]
    sorted_entities = sorted(filtered_entities, key=lambda x: custom_order.index(x[1]))
    org_keys = []
    person_keys = []
    loc_keys = []
    norp_keys = []
